<template>
    <div>
        ...
        <div v-if="product" class="border">
            ...
            <p class="ms-2">중도상환 수수료 : {{ product.erly_rpay_fee }}</p>
            <p class="ms-2">연체 이자율 : {{ product.dly_rate }}</p>
            <p class="ms-2">대출 한도 : {{ product.loan_lmt }}</p>
            <p class="ms-2">대출 인지비용 : {{ product.loan_inci_expn }}</p>
        ...
        <div v-if="options">
            <div class="card my-3" v-for="option in options">
                <br>
                <p class="ms-2">대출 상환 유형명 : {{ option.rpay_type_nm }}</p>
                <br>
                <p class="ms-2">대출 금리 유형명 : {{ option.lend_rate_type_nm }}</p>
                <br>
                <p class="ms-2">최저 대출 금리 [소수점 2자리] : {{ option.lend_rate_min }}</p>
                <br>
                <p class="ms-2">대출금리_최고 [소수점 2자리] : {{ option.lend_rate_max }}</p>
                <br>
                <p class="ms-2">전월 취급 평균금리 [소수점 2자리] {{ option.lend_rate_avg }}</p>
            </div>
        </div>
    </div>
</template>
